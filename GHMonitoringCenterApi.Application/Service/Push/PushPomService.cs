   ...
   /// </summary>
    public class PushPomService : IPushPomService
    {
        ...
        public async Task<ResponseAjaxResult<bool>> PushProjectAsync()
        {
            ...
            var orgIds = projectOrgs.Select(t => t.OrganizationId).ToArray();
            var dealUnit = await GetDealingUnitAsync(orgIds);
            ...
            foreach (var singleChangeModels in splitModels)
            {
                await SinglePushProjectAsync(option, maxChangeTime, singleChangeModels,
                    ...
                    projectConditionGrades, projectConstructionQualifications, projectRegions, projectCurrencys, users, dealUnit);
            }
        ...
        public async Task SinglePushProjectAsync(PushToPomOption option, DateTime maxChangeTime, PushChangeEntityDto<Project>[] changeModels,
            ...
            List<ProjectArea> projectRegions, List<Currency> currencys, List<Domain.Models.User> users, List<DealingUnit> dealings)
        {
            ...
            foreach (var changeModel in changeModels)
            {
                ...
                pushModels.Add(ConvertProjectRequestDto(changeModel.Entity, leaderList, orgList, dealings, typeFirst, statusFirst, gradFirst,
        ...
        /// </summary>
        /// <returns></returns>
        private ModifyInsertProjectRequestDto ConvertProjectRequestDto(Project project, List<ProjectLeader> projectLeaders, List<ProjectOrg> projectOrgs, List<DealingUnit> dealings, ProjectType projectTypes,
            ...
            ProjectArea projectAreas, Currency currency, Domain.Models.User createUser, Domain.Models.User updateUser, Institution departMent, Institution tCompany)
        {
            ...
            {
                item.ProjectId = project.MasterProjectId.ToString();
                item.OrganizationZbp = dealings.Where(t => t.PomId.ToString() == item.OrganizationId).FirstOrDefault()?.ZBP;
                if (item.Type == "13")
                {
                    item.Type = "99";
                }
        ...
        public async Task<ResponseAjaxResult<bool>> PushOwnerShipMonthReportsAsync()
        {
            ...
            var splitModels = changeModels.OrderBy(t => t.Entity.DateMonth).Split(option.SinglePushNum);
            foreach (var singleChangeModels in splitModels)
            {
        ...
        /// <param name="projectIds"></param>
        /// <returns></returns>
        public async Task<List<DealingUnit>> GetDealingUnitAsync(Guid?[] orgIds)
        {
            return await _dbDealingUnit.AsQueryable().Where(x => orgIds.Contains(x.PomId.Value) && x.IsDelete == 1).ToListAsync();
        }
        /// <summary>
        /// 获取所属公司
        /// </summary>
