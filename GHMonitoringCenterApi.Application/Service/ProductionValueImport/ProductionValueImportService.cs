...
namespace GHMonitoringCenterApi.Application.Service.ProductionValueImport
{
    ...
    public class ProductionValueImportService : IProductionValueImportService
    {
        ...
        public async Task<ResponseAjaxResult<List<ProductionDayReportHistoryResponseDto>>> ExcelJJtSendMessageAsync(ImportHistoryProductionValuesRequestDto importHistoryProductionValuesRequestDto, int needMonth)
        {
            ...
            var baseProjectInfo = await _dbContext.Queryable<ExcelCompanyProjectBasePoduction>().Where(x => x.IsDelete == 1 && x.Year == year && x.Month == month)
