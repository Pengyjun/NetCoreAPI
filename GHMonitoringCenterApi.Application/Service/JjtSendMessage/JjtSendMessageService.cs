...
namespace GHMonitoringCenterApi.Application.Service.JjtSendMessage
{
    ...
    public class JjtSendMessageService : IJjtSendMessageService
    {
        ...
        public async Task<ResponseAjaxResult<JjtSendMessageMonitoringDayReportResponseDto>> JjtTextCardMsgDetailsAsync(int dateDay = 0)
        {
            ...
            #region 
            //获取项目产值计划表数据
            var monthPlanRepData = await dbContext.Queryable<ProjectPlanProduction>().Where(x => x.IsDelete == 1).ToListAsync();
            //获取项目日报产值数据
        ...
        /// <returns></returns>
        public decimal GetProjectPlanAmount(List<ProjectPlanProduction> pPlanPro, int yearInt, int monthInt)
        {
            pPlanPro = pPlanPro.Where(x => x.Year == yearInt).ToList();
            ...
            switch (monthInt)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    planProAmount = pPlanPro.Sum(x => Convert.ToDecimal(x.ThreePlanProductionValue));
                    break;
                case 4:
                    planProAmount = pPlanPro.Sum(x => Convert.ToDecimal(x.FourPlanProductionValue));
                    break;
                case 5:
                    planProAmount = pPlanPro.Sum(x => Convert.ToDecimal(x.FivePlanProductionValue));
                    break;
                case 6:
                    planProAmount = pPlanPro.Sum(x => Convert.ToDecimal(x.SixPlanProductionValue));
                    break;
                case 7:
                    planProAmount = pPlanPro.Sum(x => Convert.ToDecimal(x.SevenPlanProductionValue));
                    break;
                case 8:
                    planProAmount = pPlanPro.Sum(x => Convert.ToDecimal(x.EightPlanProductionValue));
                    break;
                case 9:
                    planProAmount = pPlanPro.Sum(x => Convert.ToDecimal(x.NinePlanProductionValue));
                    break;
                case 10:
                    planProAmount = pPlanPro.Sum(x => Convert.ToDecimal(x.TenPlanProductionValue));
                    break;
                case 11:
                    planProAmount = pPlanPro.Sum(x => Convert.ToDecimal(x.ElevenPlanProductionValue));
                    break;
                case 12:
                    planProAmount = pPlanPro.Sum(x => Convert.ToDecimal(x.TwelvePlanProductionValue));
                    break;
            }
