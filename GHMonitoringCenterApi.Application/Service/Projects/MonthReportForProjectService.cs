    ...
    public class MonthReportForProjectService : IMonthReportForProjectService
    {
        ...
        private async Task<List<ProjectWBSDto>> GetWBSDataAsync(Guid pId, int? dateMonth, bool dayRep)
        {
            ...
            if (dateMonth != 0 && dateMonth.ToString().Length == 6)
            {
                ...
                {
                    var model = handleList.Where(t => !string.IsNullOrWhiteSpace(item.Key.ProjectWBSId.ToString()) && t.ProjectId == item.Key.ProjectId && t.ShipId == item.Key.ShipId && t.UnitPrice == item.Key.UnitPrice && t.ProjectWBSId == item.Key.ProjectWBSId).FirstOrDefault();
