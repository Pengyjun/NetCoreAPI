    ...
    public class ProjectReportService : IProjectReportService
    {
        ...
        public async Task<ResponseAjaxResult<ProjectDayReportResponseDto>> SearchProjectDayReportAsync(ProjectDayReportRequestDto model)
        {
            ...
            if (dayReport == null)
            {
                ...
                {
                    currencyExchangeRate = await GetCurrencyRateAsync(currencyId, Convert.ToInt32(dateDay.ToString().Substring(0, 4)));
                }
            ...
            if (resDayReport != null && resDayReport.Construction != null)
            {
                ...
                {
                    resDayReport.Construction.CurrencyExchangeRate = await GetCurrencyRateAsync(resDayReport.Construction.CurrencyId, Convert.ToInt32(dateDay.ToString().Substring(0, 4)));
                }
            }
            #endregion
        ...
        //         var mReportData = await _dbContext.Queryable<MonthReport>().Where(x => x.IsDelete == 1 && projectIds.Contains(x.ProjectId)).ToListAsync();
        //         var mIds = mReportData.Select(x => x.Id).ToList();
        //         var mDetailsData = await _dbContext.Queryable<MonthReportDetail>().Where(x => x.IsDelete == 1 && projectIds.Contains(x.ProjectId)).ToListAsync();

        //         var pIds = list.Select(t => t.ProjectId.ToString()).Distinct().ToList();
        //         var pHWBSLsit = await _dbContext.Queryable<ProjectWbsHistoryMonth>().Where(x => pIds.Contains(x.ProjectId)).ToListAsync();
        //         var pWBSList = await _dbContext.Queryable<ProjectWBS>().Where(x => x.IsDelete == 1 && pIds.Contains(x.ProjectId)).ToListAsync();
        //         //汇率
        ...
        //             item.YearPlanQuantities = Math.Round(sumProjectYearPlanned.PlannedQuantities);
        //             item.YearPlanOutPutValue = Math.Round(sumProjectYearPlanned.PlannedOutputValue);
        //             // 合同金额
        //             if (item.ContractAmount != null)
        //             {
        //                 item.ContractAmount = Math.Round((decimal)(model.IsConvert == true ? item.ContractAmount * (item.ExchangeRate ?? 1m) : item.ContractAmount));
        //             }
        //             // 有效合同额
        //             if (item.ValidContractAmount != null)
        //             {
        //                 item.ValidContractAmount = Math.Round((decimal)item.ValidContractAmount);
        //             }
        //             var wbsList = HandleWBSDataAsync(item.ProjectId, item.DateMonth, pHWBSLsit, pWBSList);
        //             //汇率
        ...
        //             {
        //                 #region 新的
        //                 item.YearAccomplishQuantities = val.YearAccomplishQuantities;
        //                 item.YearAccomplishCost = val.YearAccomplishCost;
        //                 item.YearAccomplishValue = val.YearAccomplishValue;
        //                 item.YearPaymentAmount = val.YearPaymentAmount;
        //                 item.YearRecognizedValue = val.YearRecognizedValue;
        //                 item.YearProjectedCost = val.YearProjectedCost;
        //                 item.YearOutsourcingExpensesAmount = val.YearOutsourcingExpensesAmount;
        //                 #endregion
        //             }
        //             // 累计统计月报
        //             if (sumMonthReport != null)
        ...
        //                 if (itemHistotyMonth != null)
        //                 {
        //                     item.CumulativePaymentAmount = val.CumulativePaymentAmount + Math.Round(itemHistotyMonth.KaileiProjectPayment.Value * 10000);

        //                     item.CumulativeValue = val.CumulativeValue + Math.Round(itemHistotyMonth.KaileiOwnerConfirmation.Value * 10000);
        //                 }
        //                 #endregion
        //             }
        //             // (国别|省）/市
        ...
        //             }
        //             // 标后预算预计成本
        //             item.PostmarkProjectedCost = Math.Round(item.CumulativeCompleted * ((100 - item.BudgetInterestRate ?? 0) / 100));
        //             // 支付滞后比例(支付滞后比例 = 合同约定的计量支付比例-实际支付比例,   实际支付比例=甲方付款金额/甲方确认产值)
        //             if (item.RecognizedValue != 0)
        //             {
        //                 item.PaymentLagRatio = Math.Round(item.ContractMeaPayProp - ((item.PaymentAmount / item.RecognizedValue) * 100));
        //             }
        //             // 实际毛利率
        //             if (item.CumulativeCompleted > 0)
        //             {
        //                 item.EffectiveGrossMargin = Math.Round(((item.CumulativeCompleted - item.CumulativeAccomplishCost) / item.CumulativeCompleted) * 100);
        //             }
        //             // 剩余合同金额
        //             item.ResidueContractAmount = Math.Round((item.ContractAmount ?? 0) - item.CumulativeCompleted);
        //             // 偏差计算
        //             //   本月进度偏差  
        //             item.DeviationQuantities = Math.Round(item.PlanQuantities - item.AccomplishQuantities);
        //             //   年度进度偏差
        //             item.YearDeviation = Math.Round(item.YearPlanQuantities - item.YearAccomplishQuantities);
        //             //   本月产值进度偏差 
        //             item.ScheduleOutPutValue = Math.Round(item.PlanOutPutValue - item.AccomplishValue);
        //             //   年度产值偏差 
        //             item.YearAnnualOutputValue = Math.Round(item.YearPlanOutPutValue - item.YearAccomplishValue);
        //             //   本月产值进度偏差
        //             item.CostDeviation = Math.Round(item.ProjectedCost - item.AccomplishCost);
        //             //   年度成本偏差
        //             item.YearCostDeviation = Math.Round(item.YearProjectedCost - item.YearAccomplishCost);
        //             //   标后预算成本偏差
        //             item.PostmarkCostDeviation = Math.Round(item.PostmarkProjectedCost - item.CumulativeAccomplishCost);
        //             //   毛利率偏差
        //             if (item.BudgetInterestRate != null && item.BudgetInterestRate != 0)
        //             {
        //                 item.GrossMarginDeviation = Math.Round(((item.EffectiveGrossMargin - (decimal)item.BudgetInterestRate) / (decimal)item.BudgetInterestRate) * 100);
        //             }
        //         });

        //         //  合计
        //         #region 新的
        //         var totalMonthtReport = new MonthtReportsResponseDto.TotalMonthtReportsDto
        //         {
        ...
        //             item.PlanOutPutValue = Math.Round(item.PlanOutPutValue, 2);
        //             item.YearPlanQuantities = Math.Round(item.YearPlanQuantities, 2);
        //             item.YearPlanOutPutValue = Math.Round(item.YearPlanOutPutValue, 2);
        ...
        //             item.YearAccomplishQuantities = Math.Round(item.YearAccomplishQuantities, 2);
        //             item.YearAccomplishCost = Math.Round(item.YearAccomplishCost, 2);
        //             item.YearAccomplishValue = Math.Round(item.YearAccomplishValue, 2);
        //             item.YearPaymentAmount = Math.Round(item.YearPaymentAmount, 2);
        ...
        //             item.CumulativePaymentAmount = Math.Round(item.CumulativePaymentAmount, 2);
        //             item.CumulativeValue = Math.Round(item.CumulativeValue, 2);
        //             item.PostmarkProjectedCost = Math.Round(item.PostmarkProjectedCost, 2);
        //             item.PaymentLagRatio = item.PaymentLagRatio != null ? Math.Round(item.PaymentLagRatio.Value, 2) : 0M;
        //             item.EffectiveGrossMargin = Math.Round(item.EffectiveGrossMargin, 2);
        //             item.ResidueContractAmount = Math.Round(item.ResidueContractAmount, 2);
        //             item.DeviationQuantities = Math.Round(item.DeviationQuantities, 2);
        //             item.YearDeviation = Math.Round(item.YearDeviation, 2);
        //             item.ScheduleOutPutValue = Math.Round(item.ScheduleOutPutValue, 2);
        //             item.YearAnnualOutputValue = Math.Round(item.YearAnnualOutputValue, 2);
        //             item.CostDeviation = Math.Round(item.CostDeviation, 2);
        ...
        public async Task<ResponseAjaxResult<MonthtReportsResponseDto>> SearchMonthReportsAsync(MonthtReportsRequstDto model)
        {
            ...
            {
                var dt = await SearchMonthReportsDateAsync(model);
                return dt;
            }
            ...
            ResolveTagNames(model.TagName, out List<int> categoryList, out List<int> tagList, out List<int> tag2List);
            RefAsync<int> total = 0;
            var query = _dbMonthReport.AsQueryable()
                .LeftJoin(_dbProject.AsQueryable(), (m, p) => m.ProjectId == p.Id && p.IsDelete == 1);
            query = MonthReportsWheres(query, model, userAuthForData, categoryList, tagList, tag2List)
            ...
            var industryClassifications = await GetIndustryClassificationsAsync();
            // 年度计划产值/产量
            var projectAnnualPlans = await GetProjectAnnualPlansAsync(projectIds);
            // 累计产值/产量
            var sumMonthReports = await SumMonthReportsAsync(projectIds);
            //项目历史确认和收款产值
            var sumHistoryMonthReports = await _dbContext.Queryable<ProjectMonthReportHistory>().Where(x => x.IsDelete == 1 && projectIds.Contains(x.ProjectId.Value)).ToListAsync();
            ...
            //25年之前不存在的历史数据
            var monthreportAdd2024 = await _dbContext.Queryable<MonthReportDetailAdd>().Where(x => x.IsDelete == 1).ToListAsync();
            var pIds = list.Select(t => t.ProjectId.ToString()).Distinct().ToList();
            var pHWBSLsit = await _dbContext.Queryable<ProjectWbsHistoryMonth>().Where(x => pIds.Contains(x.ProjectId)).ToListAsync();
            var pWBSList = await _dbContext.Queryable<ProjectWBS>().Where(x => x.IsDelete == 1 && pIds.Contains(x.ProjectId)).ToListAsync();
            //汇率
            ...
            foreach (var item in list)
            {
                ...
                int year = Convert.ToInt32(item.DateMonth.ToString().Substring(0, 4));
                var taxRate = pRates.FirstOrDefault(x => x.Year == year && x.CurrencyId == item.CurrencyId.ToString());
                var val = GetMonthReportValue(item.DateMonth, item.ProjectId, mDetailsData, mReportData, model.IsConvert, wbsList, hisMonthRep24, item.CurrencyId);
                item.AccomplishValue2 = taxRate != null ? taxRate.ExchangeRate.Value * item.AccomplishValue2 : 0M;//转换人民币
                ...
                if (item.DateMonth > 202412)
                {
                    ...
                    if (item.CurrencyId == "2a0e99b4-f989-4967-b5f1-5519091d4280".ToGuid())//国内
                    {
                        val.CumulativeCompleted = before2024.Sum(x => Convert.ToDecimal(x.ActualCompAmount)) + after2024.Sum(x => x.CompleteProductionAmount)
                            + monthreportAdd2024.Where(x => x.ProjectId == item.ProjectId).Sum(x => x.CompleteProductionAmount);
                        val.CumulativeOutsourcingExpensesAmount = before2024.Sum(x => Convert.ToDecimal(x.CurrencyOutsourcingExpensesAmount)) + after2024.Sum(x => x.OutsourcingExpensesAmount) + monthreportAdd2024.Where(x => x.ProjectId == item.ProjectId).Sum(x => x.OutsourcingExpensesAmount);
                    }
                    else
                    {
                        if (model.IsConvert == true)//切换人民币
                        {
                            val.CumulativeCompleted = before2024.Sum(x => Convert.ToDecimal(x.RMBHValue)) + after2024.Sum(x => x.CompleteProductionAmount) + monthreportAdd2024.Where(x => x.ProjectId == item.ProjectId).Sum(x => x.CompleteProductionAmount);
                            val.CumulativeOutsourcingExpensesAmount = before2024.Sum(x => Convert.ToDecimal(x.RMBHOutValue)) + after2024.Sum(x => x.OutsourcingExpensesAmount) + monthreportAdd2024.Where(x => x.ProjectId == item.ProjectId).Sum(x => x.OutsourcingExpensesAmount);
                        }
                        else
                        {
                    ...
                    val.AccumulativeQuantities = before2024.Sum(x => Convert.ToDecimal(x.ActualCompQuantity)) + after2024.Sum(x => x.CompletedQuantity) + monthreportAdd2024.Where(x => x.ProjectId == item.ProjectId).Sum(x => x.CompletedQuantity);
                }
                else
                {
                    if (model.IsConvert == false)
            ...
            var mpList = await _dbMonthReport.AsQueryable()
                ...
                .Select(x => new { x.Id, x.CompletedQuantity, x.CompleteProductionAmount, x.ProjectId, x.PartyAConfirmedProductionAmount, x.PartyAPayAmount, x.OutsourcingExpensesAmount, x.DateMonth })
                .ToListAsync();
            ...
            if (!specicalCondition)
            {
                ...
                {
                    AccomplishQuantities = Math.Round(mpList.Sum(m => m.CompletedQuantity), 2),
                    AccomplishValue = Math.Round(mpList.Sum(m => m.CompleteProductionAmount), 2),
                    RecognizedValue = Math.Round(mpList.Sum(m => m.PartyAConfirmedProductionAmount), 2),
            ...
            else
            {
                ...
                totalMonthtReport = new MonthtReportsResponseDto.TotalMonthtReportsDto
                {
                    ...
                    AccomplishValue = Math.Round(mpList.Where(x => !subpIds.Contains(x.ProjectId)).Sum(m => m.CompleteProductionAmount), 2),
                    RecognizedValue = Math.Round(mpList.Where(x => !subpIds.Contains(x.ProjectId)).Sum(m => m.PartyAConfirmedProductionAmount), 2),
                    PaymentAmount = Math.Round(mpList.Where(x => !subpIds.Contains(x.ProjectId)).Sum(m => m.PartyAPayAmount), 2),
                    OutsourcingExpensesAmount = Math.Round(mpList.Where(x => !subpIds.Contains(x.ProjectId)).Sum(m => m.OutsourcingExpensesAmount), 2)
                };
            }
            #endregion
        ...
        /// <returns></returns>
        public async Task<ResponseAjaxResult<MonthtReportsResponseDto>> SearchMonthReportsDateAsync(MonthtReportsRequstDto model)
        {
            if (model.IsDuiWai)
            {
                _currentUser.Account = "2022002867";
        ...
        /// </summary>
        /// <returns></returns>
        private MonthReportForProjectConvert GetMonthReportValue(int dateMonth, Guid projectId, List<MonthReportDetail> mDetails, List<MonthReport> mReport, bool? isConvert, List<ProjectWBSDto> wbsList, List<MonthReportDetailHistory> hisMonthRep24, Guid? CurrencyId)
        {
            /***
             * 1.截取开始年份
             //* 2.获取存在的wbsids、
        ...
        /// <param name="hWbs"></param>
        /// <param name="wbs"></param>
        /// <returns></returns>
        private List<ProjectWBSDto> HandleWBSDataAsync(Guid pId, int dateMonth, List<ProjectWbsHistoryMonth> hWbs, List<ProjectWBS> wbs)
        {
            var pWBSList = new List<ProjectWBSDto>();

            var wbsHsitoryList = hWbs.Where(p => p.ProjectId == pId.ToString())
            ...
            if (wbsHsitoryList.Any())
            {
                ...
                if (deleteMonthList != null && deleteMonthList.Any())
                {
                    ...
                    //追加最新的wbs数据
                    var nPWBSList = wbs.Where(p => !string.IsNullOrEmpty(p.ProjectId) && p.IsDelete == 1 && SqlFunc.ToGuid(p.ProjectId) == pId && !pIds.Contains(p.Id))
                                 .Select(p => new ProjectWBSDto
                                 {
                                     EngQuantity = p.EngQuantity,//wbs初始的工程量
                                     KeyId = p.KeyId,
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        //追加最新的wbs数据
                        var nPWBSList = wbs// _dbContext.Queryable<ProjectWBS>()
            ...
            pWBSList = wbs.Where(p => !string.IsNullOrEmpty(p.ProjectId) && p.IsDelete == 1 && SqlFunc.ToGuid(p.ProjectId) == pId)
               .Select(p => new ProjectWBSDto
               {
                   EngQuantity = p.EngQuantity,//wbs初始的工程量
                   KeyId = p.KeyId,
        ...
        private async Task<SumProjectPlannedDto> SumMonthProjectPlannedAsync(Guid projectId, DateTime dayTime)
        {
            var monthTime = dayTime.Day < 26 ? new DateTime(dayTime.Year, dayTime.Month, 1) : new DateTime(dayTime.AddMonths(1).Year, dayTime.AddMonths(1).Month, 1);
            var years = GetPlanYears(monthTime, monthTime);
            var plans = await GetProjectAnnualPlansAsync(new Guid[] { projectId });
            return SumMonthProjectPlanned(plans, projectId, monthTime);
        }
        ...
        //}
        #endregion
        private async Task<List<ProjectPlanProduction>> GetProjectAnnualPlansAsync(Guid[] projectIds)
        {
            return await _dbProjectAnnualPlan.GetListAsync(t => t.IsDelete == 1 && projectIds.Contains(t.ProjectId));
        }
        private async Task<List<ProjectPlanProduction>> GetProjectAnnualPlansAsync(Guid[] projectIds, int[] years)
        {
            return await _dbProjectAnnualPlan.GetListAsync(t => t.IsDelete == 1 && projectIds.Contains(t.ProjectId) && years.Contains(t.Year));
        ...
        public async Task<ResponseAjaxResult<ProjectMonthReportResponseDto>> SearchProjectMonthReportAsync(ProjectMonthReportRequestDto model)
        {
            ...
            if (status == MonthReportStatus.None || status == MonthReportStatus.ApproveReject)
            {
                ...
                if (currencyId != CommonData.RMBCurrencyId)
                {
                    currencyExchangeRate = await GetCurrencyRateAsync(currencyId, Convert.ToInt32(dateMonth.ToString().Substring(0, 4)));
                }
            ...
            {
                resMonthReport.CurrencyExchangeRate = await GetCurrencyRateAsync(resMonthReport.CurrencyId, Convert.ToInt32(dateMonth.ToString().Substring(0, 4)));
            }
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<decimal> GetCurrencyRateAsync(Guid currencyId, int year)
        {
            var currencyConverter = await _dbCurrencyConverter.AsQueryable().Where(t => t.CurrencyId == currencyId.ToString() && t.Year == year && t.IsDelete == 1).SingleAsync();
        ...
        {
            ResponseAjaxResult<ProjectOutPutValueExcelResponseDto> responseAjaxResult = new();
            ProjectOutPutValueExcelResponseDto putValueExcelResponseDto = new();

            //日期处理
            int nowMonth = 0;//当月
            int startMonth = 0;//年度使用
            int endMonth = 0;//年度使用
            if (putExcelRequestDto.SearchTime != DateTime.MinValue && !string.IsNullOrWhiteSpace(putExcelRequestDto.SearchTime.ToString()))
            {
                nowMonth = putExcelRequestDto.SearchTime.ToDateMonth();
                startMonth = Convert.ToInt32(putExcelRequestDto.SearchTime.Year + "01");
                endMonth = nowMonth;
            }
            else
            {
                if (DateTime.Now.Day >= 26)
                {
                    nowMonth = DateTime.Now.ToDateMonth();
                    startMonth = Convert.ToInt32(DateTime.Now.Year + "01");
                    endMonth = nowMonth;
                }
                else
                {
                    nowMonth = DateTime.Now.AddMonths(-1).ToDateMonth();
                    startMonth = Convert.ToInt32(DateTime.Now.AddMonths(-1).Year + "01");
                    endMonth = nowMonth;
                }
            }

            //获取需要统计的公司(从监控中心日报图中获取)
            var companyList = await _dbContext.Queryable<ProductionMonitoringOperationDayReport>()
                .Where(t => t.Name != "广航局总体" && !string.IsNullOrWhiteSpace(t.Name) && t.IsDelete == 1 && t.Type == 1)
                .Select(t => new { t.ItemId, t.Name, t.Type, t.Sort })
                .ToListAsync();
            var compIds = companyList.Select(t => t.ItemId).ToList();

            //获取公司下所有项目
            //可能需要排除的项目
            List<Guid> exceptPids = new()
            {
                "08db3b35-fb38-4bb9-8213-fe97a3275400".ToGuid()//斯里兰卡
            };
            var pjectInfos = await _dbContext.Queryable<Project>()
                .WhereIF(exceptPids != null && exceptPids.Any(), t => !exceptPids.Contains(t.Id))
                .Where(t => compIds.Contains(t.CompanyId) && t.IsDelete == 1)
                .ToListAsync();

            //部门获取
            var pDeptIds = pjectInfos.Select(t => t.ProjectDept).Distinct().ToList();
            var institutionList = _dbContext.Queryable<Institution>().Where(t => t.IsDelete == 1 && pDeptIds.Contains(t.PomId)).ToList();

            //获取月报产值产量
            var mRepList = await _dbContext.Queryable<MonthReportDetail>()
                .Where(t => t.IsDelete == 1 && t.DateMonth <= nowMonth && t.DateMonth != 202306)
                .ToListAsync();
            ...
            foreach (var item in pjectInfos)
            {
                //汇率
                decimal hvv = 0M;
                var hv = hcvList.FirstOrDefault(x => x.CurrencyId == item.CurrencyId.ToString() && x.Year == Convert.ToInt32(nowMonth.ToString().Substring(0, 4)))?.ExchangeRate ?? hvv;

                //当月自行产量
                var ownProduction = mRepList.Where(x => x.ProjectId == item.Id && x.DateMonth == nowMonth && x.OutPutType == ConstructionOutPutType.Self).Sum(x => x.CompletedQuantity);

                //当月分包产量
                var subProduction = mRepList.Where(x => x.ProjectId == item.Id && x.DateMonth == nowMonth && x.OutPutType != ConstructionOutPutType.Self).Sum(x => x.CompletedQuantity);

                //当月产量合计
                var sumProduction = mRepList.Where(x => x.ProjectId == item.Id && x.DateMonth == nowMonth).Sum(x => x.CompletedQuantity);
                ...
                //分包价差=产值合计-分包支出-自行产值
                //当月分包差价
                var subDiffValue = sumOutPutValue - subExpenditure - ownOutPutValue;

                //当年自行产量
                var yearOwnProduction = mRepList.Where(x => x.ProjectId == item.Id && x.DateMonth >= startMonth && x.DateMonth <= endMonth && x.OutPutType == ConstructionOutPutType.Self).Sum(x => x.CompletedQuantity);

                //当年分包产量
                var yearSubProduction = mRepList.Where(x => x.ProjectId == item.Id && x.DateMonth >= startMonth && x.DateMonth <= endMonth && x.OutPutType != ConstructionOutPutType.Self).Sum(x => x.CompletedQuantity);

                //当年产量合计
                var yearSumProduction = mRepList.Where(x => x.ProjectId == item.Id && x.DateMonth >= startMonth && x.DateMonth <= endMonth).Sum(x => x.CompletedQuantity);
                ...
                //分包价差=产值合计-分包支出-自行产值
                //当年分包差价
                var yearSubDiffValue = yearSumOutPutValue - yearSubExpenditure - yearOwnOutPutValue;
                ...
                if (nowMonth > 202412)
                {
                    ...
                    totalOutPutValue += monthreportAdd2024.Where(x => x.ProjectId == item.Id).Sum(x => x.CompleteProductionAmount);//手动追加的25年前的开累数
                }
                #endregion
                var state = "";
                ...
                var outPutInfo = new OutPutInfo
                {
                    ...
                    SubProduction = subProduction / 10000,
                    SumOutPutValue = sumOutPutValue / 10000,
                    SumProduction = sumProduction / 10000,
                    TotalOutPutValue = totalOutPutValue / 10000,
                    YearOwnOutPutValue = yearOwnOutPutValue / 10000,
                    YearOwnProduction = yearOwnProduction / 10000,
                    YearSubDiffValue = yearSubDiffValue / 10000,
                    YearSubExpenditure = yearSubExpenditure / 10000,
                    YearSubProduction = yearSubProduction / 10000,
                    YearSumOutPutValue = yearSumOutPutValue / 10000,
                    YearSumProduction = yearSumProduction / 10000
                };
                outPutInfos.Add(outPutInfo);
            }
            List<SumOutPutInfo> sumOutPutInfos = new();
            #region 处理分包项目产值问题
            if (_currentUser.CurrentLoginIsAdmin || _currentUser.CurrentLoginInstitutionOid == "101162350")
            {
                //获取分包项目
        ...
        public async Task<ResponseAjaxResult<ReportedMonthResponseDto>> SearchReportedMonthsAsync(ReportedMonthRequestDto model)
        {
            ...
            //查询出当前登录人 允许修改的填报月份
            var btnMonth = await _dbContext.Queryable<BtnEditMonthlyReportPermission>().Where(t => t.IsDelete == 1 && t.UserId == _currentUser.Id && t.MonthReportEnable).Select(t => t.MonthTime).FirstAsync();
            if (btnMonth != null)
            {
